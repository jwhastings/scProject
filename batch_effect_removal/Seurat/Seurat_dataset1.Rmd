---
title: "Seurat Integration (Dataset 1)"
...
plot_grid(seurat_p1, seurat_p2, nrow = 2)
...
batch <- combined@meta.data$BatchNum
...
all_rr %>%
  ...
  write_csv(file = "output/Seurat_dataset1_kBET_all.csv")
```
...
group_rr %>%
  select(group, sample_size, median_rr = `50%`) %>%
  mutate(method = "Seurat") %>%
  write_csv(file = "output/Seurat_dataset1_kBET_bygroup.csv")
```
...
all_asw %>%
  ...
  mutate(Method = "Seurat") %>%
  write_csv(file = "output/Seurat_dataset1_ASW.csv")
```

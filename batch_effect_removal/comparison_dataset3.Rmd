...
```{r}
batch_freq <- as.data.frame(table(Batch = sim_data$Batch))
batch_cluster_freq <-
  as.data.frame(table(
    Batch = sim_data$Batch,
    Cluster = sim_data$Group
  )) %>%
...
batch_cluster_df %>%
  ...
  kable_styling() %>%
  add_header_above(c("Batch" = 2, "Cluster" = 3)) %>%
...
No preprocessing for simulated data.

```{r, warning = FALSE}
# Final SCE object
data_SCE <- sim_data
colData(data_SCE)$Batch <- factor(colData(data_SCE)$Batch)
colData(data_SCE)$Cluster <- factor(colData(data_SCE)$Group)
```
...
p2_options <- scale_color_manual(name = "Cell Type", values = brew_colors[c(3:5)])
...
comparison <- compareSCEs(
  list(
    Batch1 = data_SCE[, data_SCE$Batch == unique(data_SCE$Batch)[1]],
    Batch2 = data_SCE[, data_SCE$Batch == unique(data_SCE$Batch)[2]])
  )
...
# Plot PVCA
pvca_plot <- data.frame(name = c("Batch", "Cell Type", "Residual"), values = pvca_means) %>%
  ggplot(aes(x = name, y = values)) +
  geom_col(aes(fill = name), width = 0.5, color = "black", show.legend = FALSE, linewidth = 0.25) +
  geom_label(aes(label = round(values, 3)), nudge_y = 0.05) +
  labs(x = "Effects", y = "Weighted Average Proportion Variance", title = "PVCA of Simulated Data") +
  theme(
    plot.title = element_text(face = "bold", size = rel(1.2), hjust = 0.5)
  )
pvca_plot

ggsave("output/pvca_simdata.png", pvca_plot, width = 9, height = 4, dpi = 600)
```
...
raw_svd <- svds(raw_final, k = 30)
data <- raw_svd$u %*% diag(raw_svd$d)
...
raw_pc <- raw_svd$u %*% diag(raw_svd$d)
...
# JIVE v2 runtime (given): 4478.06 sec elapsed
# JIVE algorithm converged after  708  iterations.
toc()

saveRDS(JIVE_results, file = "JIVE/data/JIVE_v2_dataset3.rds")
```

Summary of estimated ranks and variance explained by joint/individual/residual for each approach:

```{r}
JIVE_results <- readRDS(file = "JIVE/data/JIVE_v2_dataset3.rds")
...
```{r}
# data: a matrix (rows: samples, columns: features (genes))
jive_svd <- svds(jive_final, k = 30)
data <- jive_svd$u %*% diag(jive_svd$d)
...
jive_pc <- jive_svd$u %*% diag(jive_svd$d)
...
sim_metadata <- as.data.frame(colData(data_SCE))
sim_seurat <- CreateSeuratObject(counts(data_SCE), meta.data = sim_metadata)
...
sim_seurat_batch <- SplitObject(sim_seurat, split.by = "Batch")

# normalize and identify variable features for each dataset independently
sim_seurat_batch <- lapply(X = sim_seurat_batch, FUN = function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 5000)
})

# select features that are repeatedly variable across datasets for integration
features <- SelectIntegrationFeatures(object.list = sim_seurat_batch, nfeatures = 5000)
```
...
```{r}
anchors <- FindIntegrationAnchors(object.list = sim_seurat_batch, anchor.features = features)
...
seurat_integrated <- RunPCA(seurat_integrated, verbose = FALSE, npcs = 30)
...
seurat_pc <- Embeddings(seurat_integrated)
...
seurat_title <- ggdraw() + draw_label("Seurat", fontface = "bold")

seurat_tsne_plot <- plot_grid(
  seurat_title,
  seurat_tsne_batch + theme(legend.position = "none"),
  seurat_tsne_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights = c(0.1, 1, 1)
)
...
harmony_title <- ggdraw() + draw_label("Harmony", fontface = "bold")

harmony_tsne_plot <- plot_grid(
  harmony_title,
  harmony_tsne_batch + theme(legend.position = "none"),
  harmony_tsne_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights=c(0.1, 1, 1)
)
...
raw_title <- ggdraw() + draw_label("Raw", fontface = "bold")

raw_tsne_plot <- plot_grid(
  raw_title,
  raw_tsne_batch + theme(legend.position = "none"),
  raw_tsne_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights=c(0.1, 1, 1)
)
...
ggsave("output/tsne_simdata.png", all_tsne_plot, width = 9, height = 4, dpi = 600)
```
...
seurat_umap_plot <- plot_grid(
  seurat_title,
  seurat_umap_batch + theme(legend.position = "none"),
  seurat_umap_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights = c(0.1, 1, 1)
)
...
harmony_umap_plot <- plot_grid(
  harmony_title,
  harmony_umap_batch + theme(legend.position = "none"),
  harmony_umap_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights=c(0.1, 1, 1)
)
...
raw_umap_plot <- plot_grid(
  raw_title,
  raw_umap_batch + theme(legend.position = "none"),
  raw_umap_cluster + theme(legend.position = "none"),
  ncol = 1, align = "hv", rel_heights=c(0.1, 1, 1)
)
...
ggsave("output/umap_simdata.png", all_umap_plot, width = 9, height = 4, dpi = 600)
```
...
all_metrics_lot
ggsave("output/metrics_simdata.png", all_metrics_lot, width = 9, height = 4, dpi = 600)
```

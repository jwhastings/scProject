...
p2_options <- scale_color_manual(name = "Cell Type", values = RColorBrewer::brewer.pal(6, "Dark2")[1:6])
...
pvca_plot <- data.frame(name = c("Batch", "Cell Type", "Residual"), values = pvca_means) %>%
  ...
  labs(x = "Effects", y = "Weighted Average Proportion Variance", title = "PVCA of Bacher T-Cell Data") +
  theme(
    plot.title = element_text(face = "bold", size = rel(1.2), hjust = 0.5)
  )
pvca_plot
ggsave("output/pvca_bacherdata.png", pvca_plot, width = 9, height = 4, dpi = 600)
```
...
ggsave("output/tsne_bacherdata.png", all_tsne_plot, width = 9, height = 4, dpi = 600)
```
...
ggsave("output/umap_bacherdata.png", all_umap_plot, width = 9, height = 4, dpi = 600)
```
...
all_metrics_lot
ggsave("output/metrics_bacherdata.png", all_metrics_lot, width = 9, height = 4, dpi = 600)
```

jive_bic <- function (data, n = unlist(lapply(data, ncol)) * unlist(lapply(data, nrow)),
                      ...
                      orthIndiv = TRUE, showProgress = TRUE, CORES = CORES) 
{
  ...
  current <- jive_iter(data, rankJ, rankA, conv, maxiter, 
                       orthIndiv, showProgress = showProgress, CORES = CORES)
  ...
  while (bic.improve) {
    ...
    temp[[1]] <- jive_iter(data, rankJ + 1, rankA, conv, 
                           maxiter, orthIndiv, showProgress = showProgress, CORES = CORES)
    for (k in 1:length(data)) {
    ...
    for (i in 1:l) {
      ...
      if (tempR[i] < min(n, nrow(data[[i]]))) {
        ...
        temp[[i + 1]] <- jive_iter(data, rankJ, tempR, 
                                   conv, maxiter, orthIndiv, showProgress = showProgress, CORES = CORES)
        for (k in 1:length(data)) {
